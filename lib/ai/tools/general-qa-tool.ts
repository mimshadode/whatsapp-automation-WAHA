...
export class GeneralQATool implements AITool {
  name = 'General Q&A';
  description = 'Menjawab pertanyaan umum seputar Google Forms, produktivitas, dan topik terkait';
  ...
  async execute(query: string, context: ToolContext): Promise<ToolResponse> {
    try {
      console.log(`[GeneralQATool] Processing question: ${query.substring(0, 100)}...`);
      ...
      if (!hasEmbeddedContext && conversationHistory.length > 0) {
        ...
        contextStr = '\n\nKONTEKS PERCAKAPAN SEBELUMNYA:\n' + 
          recentHistory.map((h: any) => `User: ${h.user}\nBot: ${h.bot}`).join('\n');
      }
      ...
      const updatedMetadata = newBotName ? { ...currentMetadata, botName: newBotName } : currentMetadata;
      ...
      return {
        success: false,
        reply: 'Maaf, saya sedang mengalami kendala teknis. Coba tanya lagi sebentar lagi ya! 😊'
      };
    }
  }
}
